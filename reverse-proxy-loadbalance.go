// A reverse proxy that load balances between two backends in a round-robbin fashion.
...
func loadBalancingReverseProxy(target1, target2 *url.URL) *httputil.ReverseProxy {
	...
	director := func(req *http.Request) {
		var target *url.URL
		// Simple round robbin between two targets
		if targetNum == 1 {
			target = target1
			targetNum = 2
		} else {
...
func joinURLPath(a, b *url.URL) (path, rawpath string) {
	...
	aslash := strings.HasPrefix(apath, "/")
